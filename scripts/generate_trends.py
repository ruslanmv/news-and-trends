...
def generate_ml_insights(
  ...
  write_task = Task(
      description=(
          "Using the data scientist's analysis, write a comprehensive trend blog post.\n\n"
          "You MUST return your answer as JSON with this exact structure:\n\n"
          "{\n"
          '  \"title\": \"Short, punchy headline (max 120 characters)\",\n'
          '  \"body\": \"<markdown body of the article>\"\n'
          "}\n\n"
          "Guidelines for the body:\n"
          "1. Start with a short introduction (2-3 sentences).\n"
          "2. Use clear section headings, for example:\n"
          '   - \"## What’s happening\"\n'
          '   - \"## Why it matters\"\n'
          '   - \"## Emerging patterns\"\n'
          '   - \"## What to watch next\"\n'
          "3. Be professional, data-driven, and forward-looking.\n"
          "4. Do NOT fabricate facts beyond what the analysis implies.\n"
          "5. Use markdown formatting (headings, bullet lists) for readability.\n"
      ),
      expected_output='A JSON object with a "title" string and "body" markdown string.',
      agent=tech_blogger,
  ...
  try:
      data = json.loads(raw)
      title = clean_text(data.get("title", "").strip() or "AI Trend Insight")
...
def write_trend_file(trend: Dict[str, str], issue_date: str, ml_metadata: Dict[str, Any]) -> str:
  """
  ...
  title = trend.get("title") or f"AI Trends Analysis – {issue_date}"
  markdown_body = trend.get("body", "")
...
def main() -> None:
  ...
  trend = generate_ml_insights(topics, cluster_labels, temporal_trends, latest_doc)

  filepath = write_trend_file(trend, issue_date, ml_metadata)
