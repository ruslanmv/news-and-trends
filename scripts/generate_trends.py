...
def generate_ml_insights(
    ...
    write_task = Task(
        description=(
            ...
            "Structure:\n"
            "1. **Engaging headline** (will be added as H1)\n"
            "2. **Introduction** (2-3 sentences): Set the context\n"
            "3. **Current Landscape** (2 paragraphs): What the ML analysis reveals about current trends\n"
            "4. **Emerging Patterns** (2 paragraphs): Deep dive into rising trends and topic clusters\n"
            "5. **Looking Forward** (1-2 paragraphs): Predictions and what to watch for\n"
            "6. **Conclusion** (1 paragraph): Key takeaways\n\n"
            "Style:\n"
            "- Professional yet accessible\n"
            "- Data-driven but not overly technical\n"
            "- Forward-looking and insightful\n"
            "- Use specific examples from the ML insights\n\n"
            "Return ONLY the blog body in markdown (no title in body, no frontmatter)."
        ),
        expected_output="A complete trend analysis blog post in markdown format",
        agent=tech_blogger,
...
def write_trend_file(markdown_body: str, issue_date: str, ml_metadata: Dict[str, Any]) -> str:
    """
    ...
    frontmatter = (
        f"---\n"
        f'title: "AI Trends Analysis – {issue_date}"\n'
        f"date: {issue_date}\n"
        f"type: trend\n"
        f'layout: "layout.njk"\n'
        f"tags:\n"
...
def main() -> None:
    ...
    markdown = generate_ml_insights(topics, cluster_labels, temporal_trends, latest_doc)

    # Write file
    filepath = write_trend_file(markdown, issue_date, ml_metadata)
